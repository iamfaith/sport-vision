...
class PoseAnalyzerONNX:
    ...
    def __init__(
        ...
        self.output_indices = self._build_output_indices()
    ...
    def _resolve_model_path(self, model_path: Optional[str]) -> Path:
        if model_path:
            ...
            path = Path(__file__).resolve().parent.parent / "models" / "pose_landmark_lite.onnx"
        if not path.exists():
            raise FileNotFoundError(f"ONNX pose model not found at {path}")
        return path

    def _build_output_indices(self) -> dict:
        indices = {}
        for index, output in enumerate(self.session.get_outputs()):
            shape = [dim for dim in output.shape if isinstance(dim, int)]
            if shape == [1, 195]:
        ...
        if missing:
            raise RuntimeError(f"Unexpected ONNX outputs, missing: {sorted(missing)}")
        return indices
